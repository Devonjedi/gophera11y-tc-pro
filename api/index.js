...
/* ─────────────────────────────── CORS ─────────────────────────────── */
// Allowed origins: comma‑separated list in .env; in dev allow all
...
async function runPuppeteerScan(url) {
  ...
  // Use system Chromium if provided via env
...
// /scan – supports GET and POST; optional ?async=1 to queue job
const jobs = new Map();
const queue = [];
let processing = 0;
...
// Mount AI/VAPT routes (protected by requireApiKey in original if needed)
app.use(aiSummary);
app.use(aiVpat);
app.use(aiVpatPdf);

// Report and samples routes unchanged ...
